...
在玩法跑通、逻辑大致看懂之后，就可以继续在同一个对话里，把注意力放到“好不好看、好不好用”上。这里的用法和前面完全一致：你只要直接描述你想要的视觉效果和交互表现，让 AI 去改代码即可。
